...
// GET /api/stock?id_variante=1 | ?id_tienda=1
export async function GET(request: NextRequest) {
  try {
    ...
    let query = `
      SELECT ns.*, v.sku, v.nombre_variante, p.nombre as nombre_producto, t.nombre as nombre_tienda
    ...
    return errorResponse('Error al obtener el stock');
  }
}

// PUT /api/stock  — body: { id_variante, id_tienda, cantidad }
export async function PUT(request: NextRequest) {
  try {
    const { id_variante, id_tienda, cantidad } = await request.json();

    if (!id_variante || !id_tienda || cantidad === undefined) {
      return errorResponse('id_variante, id_tienda y cantidad son requeridos', 400);
    }
    ...
    const result = await pool
      ...
      .query(`
        ...
        WHEN NOT MATCHED THEN
          INSERT (id_variante, id_tienda, cantidad) VALUES (@id_variante, @id_tienda, @cantidad)
        OUTPUT INSERTED.*;
      `);
    ...
    return errorResponse('Error al actualizar el stock');
  }
}
