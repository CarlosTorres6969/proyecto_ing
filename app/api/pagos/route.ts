...
// GET /api/pagos?id=1 | ?id_pedido=1
export async function GET(request: NextRequest) {
  try {
    ...
    return errorResponse('Error al obtener los pagos');
  }
}

// POST /api/pagos  — body: { id_pedido, monto, metodo_pago, estado? }
export async function POST(request: NextRequest) {
  try {
    ...
    return errorResponse('Error al registrar el pago');
  }
}
