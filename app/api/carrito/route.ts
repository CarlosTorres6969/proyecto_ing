...
export async function GET(request: NextRequest) {
  try {
    ...
    const items = await pool
      .request()
      .input('id_carrito', sql.BigInt, carritoId)
      .query(`
...
// POST /api/carrito  — body: { id_usuario, id_variante, cantidad }
...
export async function POST(request: NextRequest) {
  try {
    ...
    return errorResponse('Error al agregar al carrito');
  }
}

// PUT /api/carrito  — body: { id_item, cantidad }
export async function PUT(request: NextRequest) {
  try {
    ...
    return errorResponse('Error al actualizar el item');
  }
}

// DELETE /api/carrito?id_item=1 | ?id_carrito=1 (vaciar)
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error('Error deleting from carrito:', error);
    return errorResponse('Error al eliminar del carrito');
  }
}
