...
// estados válidos según el schema (NVARCHAR(20))
const ESTADOS_VALIDOS = ['pendiente', 'pagado', 'enviado', 'entregado', 'cancelado'];

// GET /api/pedidos?id=1 | ?id_usuario=1 | ?estado=pendiente
export async function GET(request: NextRequest) {
  try {
    ...
    if (id_usuario) { req.input('id_usuario', sql.BigInt, id_usuario); query += ' AND id_usuario = @id_usuario'; }
    if (estado)     { req.input('estado', sql.NVarChar, estado);       query += ' AND estado = @estado'; }
    ...
    return errorResponse('Error al obtener los pedidos');
  }
}

// POST /api/pedidos  — body: { id_usuario, items: [{ id_variante, cantidad }] }
export async function POST(request: NextRequest) {
  try {
    const { id_usuario, items } = await request.json();

    if (!id_usuario || !items?.length) {
      return errorResponse('id_usuario e items son requeridos', 400);
    }
    ...
    try {
      ...
      for (const item of items) {
        const variante = await transaction
          .request()
          .input('id', sql.BigInt, item.id_variante)
          .query('SELECT precio, precio_oferta FROM variantes_producto WHERE id = @id AND activo = 1');

        if (variante.recordset.length === 0) {
          await transaction.rollback();
          return errorResponse(`Variante ${item.id_variante} no encontrada o inactiva`, 404);
        }
      ...
      const pedidoResult = await transaction
        ...
        .query('INSERT INTO pedidos (id_usuario, estado, monto_total) OUTPUT INSERTED.* VALUES (@id_usuario, @estado, @monto_total)');
      ...
      await transaction
        ...
        .query(`
          ...
          JOIN carritos_compra cc ON ic.id_carrito = cc.id
    ...
    return errorResponse('Error al crear el pedido');
  }
}

// PUT /api/pedidos  — body: { id, estado }
...
// DELETE /api/pedidos?id=1  — solo si estado = 'pendiente'
